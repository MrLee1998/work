<template>
  ...
  <!-- 商品列表 -->
  <goodsList :title="'新品上新'" :goods="newGoodses"></goodsList>
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      console.log(res);
      state.swiperList = res.carousels;
      state.newGoodses = res.newGoodses;
      state.hotGoodses = res.hotGoodses;
      state.recommendGoodses = res.recommendGoodses;
      Toast.clear();
    });
    nextTick(() => {  // 包证页面渲染完成后才会执行
