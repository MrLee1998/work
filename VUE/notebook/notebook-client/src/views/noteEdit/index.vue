...
export default {
  ...
  data() {
    return {
      ...
      title: '',
      show: false,
      actions:[{ name: '选项' }, { subname: '美食' }, { subname: '旅行' },
               { subname: '汽车' },{ subname: '时尚' },{ subname: '科技' },],
    };
  },
  methods: {
    onCancel() {
      ...
      this.$http({
        ...
        if(res.data.code == '80000') {
          this.$toast(res.data.mess)
           this.$router.push({ path: "/findNodeListByType", query: { title: this.selectCon } });
        } else {
          this.$toast(res.data.mess)
        }
      })
