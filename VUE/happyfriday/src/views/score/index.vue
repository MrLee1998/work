  ...
  <div class="score">
    <div class="your_scores_container">
      <header class="your_scores">
        <span class="score_num">{{score}}</span><span class="grade">分!</span>
      </header>
...
export default {
  ...
  computed: {
    ...mapState(['itemNum', 'itemDetail', 'ansowerId']),
    averageScore() {
    ...
    tips() {
      if( this.score == 100){
        ...
        return '很厉害哟'
      }  else if( 70 <= this.score && this.score <90) {
        return '继续加油哦'
      } else {
        return '你是不是傻！'
      }
  ...
  methods: {
    computedScore() {
      Array.from(this.ansowerId).forEach( (item) => {
        if (item) {
    ...
    getScoreTip() {
      let index = (this.score / this.averageScore) - 1
